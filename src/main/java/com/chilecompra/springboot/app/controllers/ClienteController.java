...
public class ClienteController {
	...
	public String guardar(@Valid Cliente cliente, BindingResult result, Model model,
		...
		if (!foto.isEmpty()) {
			...
			if (cliente.getId() != null && cliente.getId() > 0 && cliente.getFoto() != null
				...
				uploadFileService.delete(cliente.getFoto()); //actualiza la foto
			}
	...
	public String eliminar(@PathVariable(value = "id") Long id, RedirectAttributes flash, Locale locale) {
		...
		if (id > 0) {
			...
			if (uploadFileService.delete(cliente.getFoto())) {
				String mensajeFotoEliminar = String.format(messageSource.getMessage("text.cliente.flash.foto.eliminar.success", null, locale), cliente.getFoto()); //valida que al eliminar usuario la foto también se elimine
	...
	private boolean hasRole(String role) {
		...
		SecurityContext context = SecurityContextHolder.getContext(); //obtiene el usuario en el ámbito de seguridad

		if (context == null) {
			return false;
		}

		Authentication auth = context.getAuthentication(); //representa el objeto autenticado
		...
		return authorities.contains(new SimpleGrantedAuthority(role)); //manejo de cada rol de la colección de usuarios
