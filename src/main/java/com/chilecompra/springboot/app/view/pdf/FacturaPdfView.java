package com.chilecompra.springboot.app.view.pdf;
...
import com.chilecompra.springboot.app.models.entity.ItemFactura;
...
public class FacturaPdfView extends AbstractPdfView { //esta clase sería la vista en si
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		Factura factura = (Factura) model.get("factura"); //obtenemos la vista factura que guarda un objeto
		
		Locale locale = localeResolver.resolveLocale(request); //primera forma pasandolo a messageSource
		
		MessageSourceAccessor mensajes =  getMessageSourceAccessor(); //segunda forma que ya maneja el locale por debajo
		...
		tabla.addCell(factura.getCliente().getNombre() + " " + factura.getCliente().getApellido()); //obtenemos por medio de la relacion el nombre y apellido del cliente
		...
		document.add(tabla); //guardo las tablas en el objeto documento
		document.add(tabla2);
		
		PdfPTable tabla3 = new PdfPTable(4);
		tabla3.setWidths(new float [] {3.5f, 1, 1, 1}); //para cambiar el tamaño, medida para cada celda entre ellas
